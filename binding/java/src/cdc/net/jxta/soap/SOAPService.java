...
public class SOAPService {
	...
	public void run(){
	    System.out.println("Trying to publish a new Module Spec Advertisement (number " + count++ + ")...");
	    try{
		advertiseModuleSpec(msadv);
	    } catch(Exception e){
		System.out.println("Could not publish module spec advertisement!");
		e.printStackTrace();
	    }
     ...
     */    
    public void init(PeerGroup pg, ServiceDescriptor descriptor, StructuredTextDocument param, Policy servicePolicy) throws Exception {
	...
	if( LOG.isEnabledFor(Level.INFO) )
	    System.out.println("-> SOAPService:init() - advertiseModuleClass()");
	...
	System.out.println("Expiration time for module spec adv is " + exptime + " ms");
...
//		System.out.println("Service security policy:\t" + policyName + "\tType: " + policyType );
...
//	    }
	...
	if( LOG.isEnabledFor(Level.INFO) )
	    System.out.println("-> SOAPService:acceptOnPublicPipe(...) - waitForMessage()");
	// Listen on the pipe for a client message
	net.jxta.endpoint.Message msg = pipe.waitForMessage();
	System.out.println("-> SOAPService:acceptOnPublicPipe(...) - message arrived!");
	
	// Read the message as
	ByteArrayMessageElement msgString = (ByteArrayMessageElement)
	    msg.getMessageElement("message");
	if( LOG.isEnabledFor(Level.INFO) )		
	    System.out.println("-> SOAPService:acceptOnPublicPipe(...) - get 'message' element: \n" + msgString.toString() );
	...
	if( LOG.isEnabledFor(Level.INFO) )		
	    System.out.println("-> SOAPService:acceptOnPublicPipe(...) - get 'remote-input-pipe' element: \n" + remoteInputPipeAdvertisement.toString() );		
	...
	do {
	    try {
		System.out.print("-> SOAPService:acceptOnPublicPipe(...) - binding op with remote ip... (" + attempt +  ")\t");
	...
	System.out.println("-> SOAPService:acceptOnSecurePipe(...) - waitForMessage()");
	// Listen on the pipe for a client message
	net.jxta.endpoint.Message msg = pipe.waitForMessage();
	System.out.println("-> SOAPService:acceptOnSecurePipe(...) - message arrived!");
	...
	ByteArrayMessageElement msgString = (ByteArrayMessageElement)
	    msg.getMessageElement("message");
	if( LOG.isEnabledFor(Level.INFO) )		
	    System.out.println("-> SOAPService:acceptOnSecurePipe(...) - get 'message' element: \n" + msgString.toString() );
	
	ByteArrayMessageElement remoteInputPipeAdvertisement = (ByteArrayMessageElement) 
	    msg.getMessageElement("remote-input-pipe");
	if( LOG.isEnabledFor(Level.INFO) )		
	    System.out.println("-> SOAPService:acceptOnSecurePipe(...) - get 'remote-input-pipe' element: \n" + remoteInputPipeAdvertisement.toString() );
	...
	do {
	    try {
		System.out.print("-> SOAPService:acceptOnSecurePipe(...) - binding op with remote ip... (" + attempt +  ")\t");
		output = pipeSvc.createOutputPipe(rpa, timeout);
		System.out.println("OK");
		redo = false;
	    } catch( Exception e ) {
	...
	if( LOG.isEnabledFor(Level.INFO) )
	    System.out.println("-> SOAPService:service(...) - create MessageContext");       
	...
	if( context != null) {
		if( LOG.isEnabledFor(Level.INFO) )
		    System.out.println("-> SOAPService:service(...) - set context object");
	...
	try {		
	    ...
	    if( LOG.isEnabledFor(Level.INFO) )			
		System.out.println("-> SOAPService:service(...) - server.invoke(msgContext)");
	...
	System.out.println("-> SOAPService:service(...) - create response and return it");	
	...
	authResponseDoc.appendChild( e );
	try {
	    InputStreamMessageElement returnMessageElement = new InputStreamMessageElement("AUTHRESPONSE", 
											   MimeMediaType.XMLUTF8, authResponseDoc.getStream(), null);			
	...
	this.secureInputPipeList.add( (InputPipe) pipeSvc.createInputPipe( pipeadv ) );
