...
public class SOAPService {
	...
	private ModuleSpecAdvertisement createModuleSpecAdv(ModuleClassID mcID,
		...
		if (param == null) {
			...
			if (this.getServiceDescriptor().isSecure()) {
				LOG.info("Service security policy:\t" + policyName + "\tType: "
						+ policyType);
	...
	private void advertiseModuleSpec(ModuleSpecAdvertisement msadv)
		 ...
		 * LOG.info("Expiration time for module spec adv is " + exptime + "
		 * ms");
		 * 
		 * advPublishTask = new AdvPublishTask(msadv); publishtimer = new
		 * Timer(); publishtimer.schedule(advPublishTask, exptime, exptime);
		 */
	}
	...
	private class AdvPublishTask extends TimerTask {
		...
		public void run() {
			LOG
					.info("Trying to publish a new Module Spec Advertisement (number "
							+ count++ + ")...");
			try {
	...
	public void acceptSingleThread(InputPipe pipe) throws Exception {
		...
		if (LOG.isEnabledFor(Level.INFO))
			LOG
					.info("-> SOAPService:acceptOnPublicPipe(...) - get 'message' element: \n"
							+ msgString.toString());
		...
		if (LOG.isEnabledFor(Level.INFO))
			LOG
					.info("-> SOAPService:acceptOnPublicPipe(...) - get 'remote-input-pipe' element: \n"
		...
		do {
			try {
				LOG
						.info("-> SOAPService:acceptOnPublicPipe(...) - binding op with remote ip... ("
								+ attempt + ")\t");
				...
				LOG.warn(
						" Exception in remote binding phase! TIMEOUT expired!",
						e);
				attempt++;
		...
		if (attempt < 5) {
			...
			if (this.getServiceDescriptor().isSecure() == true) {
				...
				if (policyType.equals("TLS-based")) {
					...
					Object[] clientAuthRequestParams = policyManager
							.extractParams(policyName, authRequestDoc);
					LOG.info("OK");
					LOG.info(" Checking provided parameters correctness...");
					boolean result = policyManager.checkParams(policyName,
							clientAuthRequestParams);
					...
					if (result == true) {
						...
						// **********************************************
						LOG.info("OK!");
						PSEMembershipService pseSvc = (PSEMembershipService) pg
								.getMembershipService();
						ServiceCertificateManager certManager = new ServiceCertificateManager(
								pseSvc.getPSEConfig());
						...
						X509Certificate clientCert = certManager
								.extractCertificatesFromAdv(clientAdv)[0];
						...
						authUsers.put(clientAdv.getPeerID().toString(),
								policyManager
										.getAuthenticationParam(policyName));
						authResult = "Accepted";
						...
						if (LOG.isEnabledFor(Level.INFO))
							LOG
									.info("-> Created secure pipe for service invocation from peer '"
											+ clientAdv.getName()
											+ "': \n[Pipe ID]: "
											+ securePipeAdv.getPipeID());
						// Start a new thread for service invocation purpose
						// TODO: check if a peer has already been served?
						LOG
								.info("-> SOAPService:acceptOnPublicPipe(...) - Start InvokingThread");
						...
						LOG.info("FAILED!");
						LOG
								.info("Client cannot be authenticated for invocating service!");
						authResult = "Refused";
					}

					// Build the response message
					net.jxta.endpoint.Message returnMessage = this
							.createAuthResponseMessage(authResult, pg
									.getPeerAdvertisement(), securePipeAdv);

					LOG
							.info("-> SOAPService:acceptOnPublicPipe(...) - send back response message");
					output.send(returnMessage);
					output.close();
				} else if (policyType.equals("WSS-based")) {
					// ************* MESSAGE SECURITY BASED POLICY
					// **************
					// NOTE: process the WSS compliant message stored in
					// msgString byte array
					// Convert the received message element to a standard AXIS
					// message
					Message wssMsg = new Message(msgString.toString());
					String authResult = null;

					LOG
							.info(" Extracting WSS Auth Client Request parameters...");
					Object[] clientAuthRequestParams = policyManager
							.extractParams(policyName, wssMsg);
					LOG.info("OK");
					...
					if (result == true) {
						...
						authUsers.put(rpa.getDescription(), policyManager
								.getAuthenticationParam(policyName));
						authResult = "Accepted";
						...
						String soapMsgFinal = soapMsgFull.replaceAll(
								"http://localhost:8080/"
										+ this.serviceDescriptor.getName(),
								this.serviceDescriptor.getName());
						// Replace all must understand header specifications
						soapMsgFinal = soapMsgFinal.replaceAll(
								"mustUnderstand=\"1\"", "mustUnderstand=\"0\"");
	...
	public void acceptMultiThread(InputPipe pipe) throws Exception {
		...
		if (LOG.isEnabledFor(Level.INFO))
			LOG
					.info("-> SOAPService:acceptOnSecurePipe(...) - get 'message' element: \n"
							+ msgString.toString());
		...
		if (LOG.isEnabledFor(Level.INFO))
			LOG
					.info("-> SOAPService:acceptOnSecurePipe(...) - get 'remote-input-pipe' element: \n"
							+ remoteInputPipeAdvertisement.toString());
		...
		do {
			try {
				LOG
						.info("-> SOAPService:acceptOnSecurePipe(...) - binding op with remote ip... ("
								+ attempt + ")\t");
				output = pipeSvc.createOutputPipe(rpa, timeout);
				LOG.info("OK");
				redo = false;
	...
	/**
	 * Serve the request and send the response message (this method name is ugly -
	 * it's better to have a unique method called serve)
	 */
	private void invokeService(OutputPipe output, String requestMessage)
		...
		LOG
				.info("-> SOAPService:invokeService(...) - send back response message");
		output.send(returnMessage);
		output.close();
	}
